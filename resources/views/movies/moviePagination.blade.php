...
@if($numPages > 1)
    ...
    <div class="visible-xs visible-sm text-center">
        <ul class="pagination">
            @if($results['page'] > 1)
                <li>
                    <a href="/find?search={{ $search }}&type={{ $type }}&page={{ $results['page'] - 1 }}">prev</a>
                </li>
            @endif
                <li class="active"><a href="#">{{ $results['page'] }}</a></li>
            @if($results['page'] < $numPages)
                <li>
                    <a href="/find?search={{ $search }}&type={{ $type }}&page={{ $results['page'] + 1 }}">next</a>
                </li>
            @endif
        </ul>
    </div>
    ...
    <div class="hidden-xs hidden-sm text-center">
        <ul class="pagination">
            ...
            @if($numPages <= 10)
                @for($ii = 1;$ii <= $numPages;$ii++)
                    <li{{ ($results['page']==$ii) ? ' class=active' : '' }}>
                        <a href="/find?search={{ $search }}&type={{ $type }}&page={{ $ii }}">{{ $ii }}</a>
                    </li>
                @endfor
            @elseif($results['page'] <= 6)
                ...
                <li>
                    <a href="/find?search={{ $search }}&type={{ $type }}&page={{ $numPages }}">{{ $numPages }}</a>
                </li>
            @elseif($results['page'] >= $numPages - 5 )
                <li>
                    <a href="/find?search={{ $search }}&type={{ $type }}&page=1">1</a>
                </li>
                <li class="disabled"><a href="#">...</a></li>
